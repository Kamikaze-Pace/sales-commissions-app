...
public class SelectionWindow extends JDialog {
	...
	protected void okButtonPressed(ActionEvent evt) {
		if(totalSalesCheckBox.isSelected())
			 totalSales = selectedSalesman.calculateTotalSales();
		else
			totalSales = -1;
		
		if(totalItemsCheckBox.isSelected())
			totalItems = selectedSalesman.calculateTotalItems();
		else
			totalItems = -1;
		
		if(shirtRadio.isSelected())
			shirtSales = selectedSalesman.calculateShirtsSales();
		else
			shirtSales = -1;
		
		if(skirtRadio.isSelected()  )
			skirtSales = selectedSalesman.calculateSkirtsSales();
		else 
			skirtSales = -1;
		
		if(coatRadio.isSelected())
			coatsSales = selectedSalesman.calculateCoatsSales();
		else 
			coatsSales = -1;
		
		if(trousersRadio.isSelected())
			trousersSales = selectedSalesman.calculateTrouserSales();
		else 
			trousersSales = -1;
		
		if(commissionCheckBox.isSelected())
			commission = selectedSalesman.calculateCommission();
		else
			commission = -1;
